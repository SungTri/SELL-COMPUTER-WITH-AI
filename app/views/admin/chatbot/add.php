...
<main class="flex-1 w-full flex flex-col h-screen overflow-y-auto bg-[#F8F9FB]">
    ...
    <header class="h-20 bg-white border-b border-outline-variant flex items-center justify-between px-10 sticky top-0 z-10">
        <div class="flex items-center gap-4">
            ...
            <h1 class="text-h2 font-bold text-primary">Thêm dữ liệu Chatbot</h1>
        </div>
    ...
    <div class="p-10 w-full max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl border border-outline-variant shadow-sm overflow-hidden">
            <form action="<?php echo URLROOT; ?>/admin/storeChatbotData" method="POST" class="p-8 space-y-6">
                <?php echo csrf_field(); ?>
                <div class="space-y-2">
                    <label class="text-[14px] font-bold text-primary">Câu hỏi khách hàng thường gặp</label>
                    <input name="question" required class="w-full px-4 py-3 rounded-xl border border-outline-variant focus:border-secondary focus:ring-4 focus:ring-secondary/10 transition-all outline-none text-body-md" placeholder="Ví dụ: Shop có ship COD không?" type="text"/>
                </div>

                <div class="space-y-2">
                    <label class="text-[14px] font-bold text-primary">Câu trả lời của Bot</label>
                    <textarea name="answer" required rows="5" class="w-full px-4 py-3 rounded-xl border border-outline-variant focus:border-secondary focus:ring-4 focus:ring-secondary/10 transition-all outline-none text-body-md" placeholder="Nhập câu trả lời mà Bot sẽ phản hồi..."></textarea>
                </div>

                <div class="space-y-2">
                    <label class="text-[14px] font-bold text-primary">Từ khóa (Ngăn cách bởi dấu phẩy)</label>
                    <input name="keywords" class="w-full px-4 py-3 rounded-xl border border-outline-variant focus:border-secondary focus:ring-4 focus:ring-secondary/10 transition-all outline-none text-body-md" placeholder="Ví dụ: ship, cod, thanh toán, nhận hàng" type="text"/>
                ...
                <div class="pt-4 flex gap-4">
                    <button type="submit" class="flex-1 bg-primary text-white py-3 rounded-xl font-bold hover:bg-secondary transition-all shadow-lg shadow-primary/20">Lưu dữ liệu</button>
