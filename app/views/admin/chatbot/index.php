        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-outline-variant shadow-sm">
            <form action="<?php echo URLROOT; ?>/admin/chatbot" method="GET" class="flex-1 max-w-lg flex gap-3">
                <div class="relative flex-1">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($data['search']); ?>" placeholder="Tìm kiếm câu hỏi, câu trả lời, từ khóa..." class="w-full pl-10 pr-4 py-2.5 bg-gray-50 border border-outline-variant rounded-xl text-sm outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all"/>
                ...
                <?php if(!empty($data['search'])): ?>
                    <a href="<?php echo URLROOT; ?>/admin/chatbot" class="px-4 py-2.5 border border-outline-variant text-sm font-bold text-on-surface-variant rounded-xl hover:bg-gray-50 transition-all flex items-center justify-center">Xóa lọc</a>
                <?php endif; ?>
            </form>
